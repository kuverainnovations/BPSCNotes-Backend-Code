...
export class TierRoomsService {
  ...
  async assignDefaultTier(userId: string) {
    ...
    await this.db.query(`UPDATE users SET room_tier_id=$1 WHERE id=$2`, [starter[0].id, userId]);
  }
...
export class TierRoomsCronService {
  ...
  async closeExpiredSessions() {
    ...
    const tierKeyById = new Map<string, string>(tiers.map((t: any) => [t.id, t.tier_key]));
